...
const GraficTicketPromedioLine = () => {
  ...
  const formatRetrievedData = (data) => {
    ...
    Object.keys(data).forEach((agencia) => {
      Object.values(data[agencia]).forEach((entry) => {
        if (entry.Fecha && entry["Ticket Promedio"]) { // Cambiar a "Ticket Promedio"
          ...
          if (isValid(fecha)) {
            formattedData.push({
              ...
              Agencia: agencia,
              TicketPromedio: parseFloat(entry["Ticket Promedio"]) || 0, // Cambiar a "TicketPromedio"
            });
          }
        }
      });
          ...
          >
            ...
            {[...new Set(filteredData.map(d => d.Vendedor))].map((vendedor, index) => (
              <Line
                key={vendedor}
                type="monotone"
                dataKey="TicketPromedio" // Cambiar a "TicketPromedio"
...
export default GraficTicketPromedioLine;