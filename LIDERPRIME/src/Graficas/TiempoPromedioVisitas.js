...
const GraficHoraInicio = () => {
  ...
  useEffect(() => {
    ...
    onValue(dbRef, (snapshot) => {
        if (snapshot.exists()) {
            ...
            const horaInicioPorAgencia = calcularHoraInicioVisitas(filteredData);
  ...
  const calcularHoraInicioVisitas = (firebaseData) => {
    ...
    Object.keys(firebaseData).forEach((agencia) => {
      ...
      Object.values(registros).forEach((registro) => {
        if (!registro["Fecha"] || !registro["Tiempo Promedio de Visitas"] || !registro["Vendedor "]) return;

        let fecha = new Date(registro["Fecha"]);

        let horaInicioStr = registro["Tiempo Promedio de Visitas"];
        if (typeof horaInicioStr !== "string") {
          console.warn("⚠️ Advertencia: El valor de la hora no es una cadena:", horaInicioStr);
          return;
        }

        let horaInicio = convertirHoraASegundos(horaInicioStr);
        let horaDecimal = horaInicio / 3600;
  ...
  const convertirHoraASegundos = (hora) => {
    if (!hora || typeof hora !== "string") {
      console.error("❌ Error: Valor de hora inválido", hora);
  ...
  const convertirSegundosAMinutos = (segundos) => {
    ...
    return `${String(minutos).padStart(2, "0")}:${String(segRestantes).padStart(2, "0")}`;
  };
      ...
      <CardContent>
        ...
        <ResponsiveContainer width="100%" height={600}>
          <BarChart data={procesarDatos().datos} margin={{ top: 50, bottom: 20 }}>
            ...
            />
            <YAxis
              tickFormatter={(tick) => convertirDecimalAHora(tick)} // Convierte el valor a HH:mm:ss
              domain={[0, 10 / 60]} // De 0 a 10 minutos en horas decimales (10/60)
