...
const GraficaCompleteRuta = () => {
    ...
    const agenciasValidas = [...new Set(firebaseData.map((d) => d.Agencia))];
    ...
    useEffect(() => {
        ...
        onValue(dbRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                console.log("📥 Datos crudos desde Firebase:", data);
                let formattedData = formatRetrievedData(data);
    ...
    const formatRetrievedData = (data) => {
        let formattedData = [];
        Object.keys(data)
            .filter((agencia) => agencia !== "Data") // Filtrar "Data"
            .forEach((agencia) => {
                console.log(`🏢 Agencia detectada: ${agencia}`);
                Object.values(data[agencia]).forEach((entry) => {
                    if (entry.Fecha && entry["Cumplimiento de Ruta"]) {
                        ...
                        if (isValid(fecha)) {
                            formattedData.push({
                                ...
                                Cumplimiento: typeof entry["Cumplimiento de Ruta"] === "string"
                                    ? parseFloat(entry["Cumplimiento de Ruta"].replace("%", ""))
                                    : typeof entry["Cumplimiento de Ruta"] === "number"
                                        ? entry["Cumplimiento de Ruta"] * 100
                                        : 0,
                            });
                        }
                    }
