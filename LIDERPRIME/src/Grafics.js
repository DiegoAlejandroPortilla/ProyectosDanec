...
const ExcelUploader = () => {
  ...
  const handleUploadToFirebase = async () => {
    ...
    try {
      for (const sheetName in excelData) {
        const sheetRef = ref(database, sheetName); // 📌 Nombre de la tabla = Nombre de la hoja

        // 🔹 Obtener los datos actuales de Firebase antes de subir
        const snapshot = await get(sheetRef);
        const existingData = snapshot.exists() ? snapshot.val() : {};

        // 🔹 Convertir los datos existentes en un Set de claves únicas ("Fecha" + "Vendedor")
        const existingKeys = new Set(
          Object.values(existingData).map(row => `${row.Fecha}_${row["Vendedor "]}`)
        );

        // 🔹 Filtrar solo los datos nuevos (los que no están en Firebase)
        const newData = excelData[sheetName].filter(row => {
          const uniqueKey = `${row.Fecha}_${row["Vendedor "]}`;
          return !existingKeys.has(uniqueKey);
        });

        // 🔹 Subir solo los datos nuevos a Firebase
        for (const row of newData) {
          await push(sheetRef, row);
        }

        console.log(`✅ ${newData.length} nuevos registros añadidos a ${sheetName}`);
      }

      alert("Datos subidos exitosamente a Firebase.");
      setExcelData({});
      setFile(null);
      setOpen(false); // Cerrar modal tras la subida
    } catch (error) {
      console.error("Error al subir datos:", error);
      alert("Ocurrió un error al subir los datos.");
    } finally {
      setLoading(false); // Ocultar icono de carga
    }
  };
  ...
  useEffect(() => {
    ...
    onValue(dbRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const formattedData = formatRetrievedData(filteredData);
        setFirebaseData(formattedData);

        // 🔹 Encontrar la última fecha
        let fechas = [];

        Object.values(filteredData).forEach((tabla) => {
          Object.values(tabla).forEach((registro) => {
            if (registro["Fecha"]) {
        ...
        // 🔹 Convertir a Date y obtener la más reciente
        if (fechas.length > 0) {
          const fechasConvertidas = fechas.map(f => new Date(f)).filter(f => !isNaN(f));
          const ultima = new Date(Math.max(...fechasConvertidas));
          ultima.setDate(ultima.getDate() + 1);
          setUltimaFecha(format(ultima, "yyyy-MM-dd"));
        }
  ...
  const handleMaximize = (title) => {
    if (maximizedGraph === title) {
      setMaximizedGraph(null); // Minimizar si ya está maximizada
    } else {
      setMaximizedGraph(title); // Maximizar la gráfica seleccionada
    }
  };
    ...
    <div style={{ padding: "20px" }}>
      ...
      <Box display="flex" justifyContent="center" alignItems="center" my={2}>
        <Typography variant="h6">
          Última fecha de modificación: {ultimaFecha || "Cargando..."}
        </Typography>
      </Box>
      ...
      <Grid container spacing={2} alignItems="stretch">
        {[
          {
            ...
            pairTitle: "Gráfica de Cumplimiento de Ruta (Diario)",
            state: maximizedLeft,
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Cumplimiento de Ruta",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Efectividad de Visitas (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Efectividad de Visitas (50m)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica Efectividad de Ventas (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Efectividad de Ventas",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Ventas (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio en dólares de Avance de Venta Diaria",
          ...
          },
          {
            ...
            pairTitle: "Gráfica Ticket Promedio (Diario)",
            state: maximizedTicket,
            setState: setMaximizedTicket,
            oppositeState: maximizedTicketLine, // Estado opuesto (gráfica de línea)
            setOppositeState: setMaximizedTicketLine // Función para ocultar la gráfica de línea
          },
          {
            ...
            pairTitle: "Gráfica de Ticket Promedio",
            state: maximizedTicketLine,
            setState: setMaximizedTicketLine,
            oppositeState: maximizedTicket, // Estado opuesto (gráfica de barras)
          ...
          },
          {
            ...
            pairTitle: "Gráfica Hora Inicio (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Hora Inicio",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Hora de fin (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Hora Fin",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Horas Trabajadas (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Horas Trabajadas",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Tiempo de Visitas (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Tiempo de Visitas",
          ...
          },
          {
            ...
            pairTitle: "Clientes Planificados (Diario)",
          ...
          },
          {
            ...
            pairTitle: "Gráfica de Promedio de Clientes Planificados",
        ...
        .filter(({ title }) => title.toLowerCase().includes(filterText.toLowerCase()))
        .map(({ title, component, pairTitle }) => (
          <Grid
            key={title}
            item
            xs={12}
            sm={
              maximizedGraph === title
                ? 12 // Si está maximizada, ocupa 12 columnas
                : maximizedGraph === pairTitle
                ? 0 // Si su pareja está maximizada, se oculta (0 columnas)
                : 6 // Si no, ocupa 6 columnas
            }
            md={
              maximizedGraph === title
                ? 12 // Si está maximizada, ocupa 12 columnas
            ...
            sx={{
              ...
              display:
                maximizedGraph === pairTitle ? "none" : "block", // Oculta si su pareja está maximizada
            }}
          >
            <Card sx={{ p: 3, height: "100%", position: "relative" }}>
              ...
              >
                ...
                >
                  {maximizedGraph === title ? <Remove fontSize="small" /> : <Add fontSize="small" />}
                </IconButton>
              </Box>
